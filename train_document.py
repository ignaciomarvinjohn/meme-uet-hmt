...
from scripts.libs.model import PassLayer, TaskAPredictLayerH, TaskBPredictLayerH, TaskCPredictLayerH
...
def create_model(memotion_dataset):
    ...
    model['ta_sen'] = TaskAPredictLayerH(emb_size, in_size, 4, memotion_dataset.num_labels['sentiment'])
    model['ta_sen'].to(device)
    
    # add emotion predictor
    for part in ['tb_hum', 'tb_sar', 'tb_off', 'tb_mot']:
        model[part] = TaskBPredictLayerH(emb_size, in_size, 1)
        model[part].to(device)
        
    # add intensity predictor
    for part, task in zip(['tc_hum', 'tc_sar', 'tc_off'], ['humour', 'sarcastic', 'offensive']):
        model[part] = TaskCPredictLayerH(emb_size, in_size, memotion_dataset.num_labels[task])
        model[part].to(device)
...
def validate_model(memotion_number, model, memotion_dataloader):
    ...
    with torch.no_grad():
        ...
        for data, labels in memotion_dataloader.val_dataloader:
            ...
            logits = torch.cat((torch.squeeze(logits, 1), torch.unsqueeze(tc_hum_preds, 1)), dim=1)
            logits = torch.cat((torch.squeeze(logits, 1), torch.unsqueeze(tc_sar_preds, 1)), dim=1)
            logits = torch.cat((torch.squeeze(logits, 1), torch.unsqueeze(tc_off_preds, 1)), dim=1)
            logits = torch.cat((torch.squeeze(logits, 1), torch.unsqueeze(tb_mot_preds, 1)), dim=1)
            
            # predict for task A
            sen_preds, sen_labels = get_predictions_and_labels(model, 'ta_sen', torch.unsqueeze(logits, 1), labels[ : , 0])
...
def test_model(memotion_number, model, memotion_dataloader):
    ...
    with torch.no_grad():
        ...
        for data in memotion_dataloader.test_dataloader:
            ...
            # predict for task A
            sen_preds = get_predictions(model, 'ta_sen', torch.unsqueeze(logits, 1))
            all_preds['ta_sen'].extend(sen_preds)
...
def load_and_preprocess(memotion_number, memotion_dataset):
    ...
    # document and keyword extraction (multi task)
